...
namespace CrawlFeaturizer.Util
{
    public class CognitiveTextAnalyzer
    {
        ...
        public async Task<IReadOnlyCollection<string>> GetKeyPhrasesAsync(string document)
        {
            Response<IReadOnlyCollection<string>> results = await textAnalyticsClient.ExtractKeyPhrasesAsync(document, "en");
